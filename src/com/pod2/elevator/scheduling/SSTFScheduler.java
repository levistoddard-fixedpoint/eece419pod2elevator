...
public class SSTFScheduler implements ElevatorScheduler{
	...
	public String getKey() {
		return SSTFScheduler.class.getName();
	}

	@Override
	public String getName() {
		return "Shortest-Seek-Time-First Scheduler";
	}
	...
	/*
	if(Math.abs(elevators[j].getPosition()-i) < Math.abs(elevators[nearestElevator].getPosition()-i)){
		nearestElevator = j;
	}else if(Math.abs(elevators[j].getPosition()-i) == Math.abs(elevators[nearestElevator].getPosition()-i)){
		if(elevators[j].getRequestPanel().getRequestSize() < elevators[nearestElevator].getRequestPanel().getRequestSize()){
			nearestElevator = j;
		}
	}
	*/
	...
	public void schedule(ActiveSimulation simulation) {
		 ...
		 * 	will first search delegate floor requests to nearest elevators.
		 * 	If more than one elevators has the same distance, scheduler will assign floor to an
		 * 	elevator with the fewest requests followed by lowest id.  Only elevators that are in
		 *  service will be delegated.
		 */
			...
			if ((requests[floor].isUpSelected()
				...
				for (int j = 0; j < elevators.length; j++) {
					if (elevators[j].getServiceStatus().equals(
						...
						if(Math.abs(elevators[j].getPosition()-floor) < Math.abs(elevators[nearestElevator].getPosition()-floor)){
							nearestElevator = j;
						}else if(Math.abs(elevators[j].getPosition()-floor) == Math.abs(elevators[nearestElevator].getPosition()-floor)){
							if(elevators[j].getRequestPanel().getRequestSize() < elevators[nearestElevator].getRequestPanel().getRequestSize()){
								nearestElevator = j;
							}
						}
