...
public class PutOutOfService extends ElevatorEvent {
	...
	PutOutOfService(EventSource source, long quantum, int elevatorNumber, String reason) {
		/**
	...
	public void apply(ActiveSimulation simulation) {
		 ...
		 * 
		 * EFFECTS: Apply
		 * service event to elevator out of service.
		 */
	...
	public boolean canApplyNow(ActiveSimulation simulation) {
		/**
		 * EFFECTS: Return true if the elevator is currently empty, false otherwise..
		 */
		Elevator elevator = simulation.getElevators()[getElevatorNumber()];
		return elevator.getRequestPanel().getRequestedFloors().isEmpty();
	}
	...
	public String toString() {
		 ...
		 */
		return String.format("%s put out of service. %s", super.toString(), reason);
	}
