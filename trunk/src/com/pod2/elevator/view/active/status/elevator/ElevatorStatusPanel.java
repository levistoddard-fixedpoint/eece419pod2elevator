...
public class ElevatorStatusPanel extends JPanel {
	...
	public void statusUpdate(double position, Set<Integer> floorsOffLimit,
			int numberRequests, int requestCapacity, MotionStatus motionStatus,
			ServiceStatus serviceStatus,
			TreeMap<String, Boolean> componentFailure) {
		currentPositionDisplay.setText(Double.toString(position));

		floorsOffLimitDisplay.setText("");
		floorsOffLimitDisplay.setPreferredSize(new Dimension(160,
				floorsOffLimit.size() * 21 + 3));
		...
		componentFailureDisplay.setPreferredSize(new Dimension(142,
				componentFailure.size() * 2 * 21 + 3));
